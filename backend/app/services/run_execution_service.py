...
class RunExecutionService:
    ...
    def _retry_surgical_chapter_repair(self, tasks: list[dict]) -> bool:
        ...
        for task in tasks:
            ...
            editorial = False
            if not repaired["changes"] and editorial_eligible:
                repaired = thesis_chapter_service.editorial_repair(
                    task, task["outputs"][-1], task.get("review_result") or {},
                )
                repaired["unresolved"] = []
                editorial = bool(repaired["changes"])
    ...
    def _retry_advisor_exact_cleanup(self, tasks: list[dict]) -> bool:
        """Apply a passed-gate advisor's explicit quoted deletion once, without rewriting."""
        changed = False
        for task in tasks:
            review = task.get("review_result") or {}
            feedback = str(review.get("feedback") or "")
            if not (
                ...
                and task.get("status") == "failed"
                and (review.get("quality_gates") or {}).get("passed") is True
                and not self._has_task_event(task, "revision.advisor_exact_cleanup")
                and task.get("outputs")
            ):
                continue
            ...
            run_event_service.emit(
                task["run_id"], "revision.advisor_exact_cleanup", "review",
                "精确执行导师点名删除",
                "仅删除导师引号内且在点名段落中逐字存在的原文；随后重跑全部质量门。",
                task_id=task["id"], agent_id=task.get("owner_agent"),
                payload={"changes": repaired["changes"]},
            )
            changed = True
        return changed
